...
fn test_git_error_display() {
    use quanttide_devops::source::git_tag::TagError;
...
fn test_git_verify_version() {
    ...
    let result = quanttide_devops::contract::verify_version(d.path(), &scope);
...
"#,
    ...
    .unwrap();
    let vs = quanttide_devops::contract::verify_version(d.path(), &scope).unwrap();
